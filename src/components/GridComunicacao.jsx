...
export default function GridComunicacao({ escalas, datas, loading, onRemover, podeEditar, theme: t }) {
  ...
  if (!datas || datas.length === 0)
    return <div style={{ padding: "60px", textAlign: "center", color: t.textMuted, fontSize: "14px" }}>Nenhuma data disponível para este mês</div>;
      ...
      <table style={{ width: "100%", minWidth: "600px", borderCollapse: "collapse", fontSize: "13px" }}>
        ...
        <tbody>
          {datas.map((dataObj, idx) => {
            ...
            return (
              <tr key={idx} style={{ borderBottom: `1px solid ${t.border}`, background: idx % 2 === 0 ? t.bg : t.surface, transition: "background 0.1s" }}>
